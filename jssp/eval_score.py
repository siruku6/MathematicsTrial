...
def assign_task_to_free_time(
    ...
    if (OperationTypeMapping is not None) and (OperationTypeMapping[gene] == "open"):
        ...
        available_free_times: list = [
            ...
            if (
                (free_time["machine_type_id"] == m_type_id)
        ...
        if len(available_free_times) > 0:
            ...
            machine_id = occupied_free_time["machine_id"]
...
def calc_time4job(
    ...
    len(target_chromosome) => num_job * num_machine_types
    """
    j_keys: list[int] = [j for j in range(num_job)]
    m_keys: list[int] = [j for j in range(0, num_machine_types)]
    ...
    for machine_type_id in m_keys:
        ...
        for _machine_id in range(0, machine_num_per_type[machine_type_id]):
            time4machine[machine_type_id + 1][_machine_id] = 0
    ...
    for gene in target_chromosome:
        ...
        if assign_result["success"]:
            ...
            machine_id = assign_result["machine_id"]
        # タスクを担当する機械ID決める (今まで処理時間が最も短い機械)
        else:
            # OPTIMIZE: ここでは、余計な待機時間が少なくて済む機械を選びたいが、暫定で単純な処理とした
            machine_id = min(time4machine[m_type_id], key=time4machine[m_type_id].get)
            ...
            if time4machine[m_type_id][machine_id] < time4job[gene]:
                ...
                free_times.append(
                    {
                        "start_time": time4machine[m_type_id][machine_id],
                        "end_time": time4job[gene],
                        "interval": time4job[gene]
                        - time4machine[m_type_id][machine_id],
...
def prepare_gannt_data_from(
    ...
    for gene in sequence_best:
        ...
        if assign_result["success"]:
            free_times = assign_result["remained_free_times"]
            machine_id: int = assign_result["machine_id"]
        ...
        j_record = j_record | make_job_record(
            end_time_sec, gene, gen_t, m_type_id, machine_id, j_record
        )
        done_count_in_job[gene] = done_count_in_job[gene] + 1
